...
select_button = ttk.Button(root, text="Select csv File",
                           command=select_file, width=25, style='GreyButton.TButton')

# place the button in the window using the grid layout manager
select_button.place(relx=0.50, rely=0.33, anchor=CENTER)
style = ttk.Style()
style.configure('WhiteButton.TButton', background='white', foreground='black',
                borderwidth=4, relief='raised', font=('Arial', 10, 'bold'))
...
def download_dir():
    ...
    select_dir = fd.askdirectory(
        title="Select Download Location", initialdir="/")
...
download_button = ttk.Button(root, text="Select Download Location",
                             command=download_dir, width=25, style='GreyButton.TButton')
...
close_button.config(height=int(
    close_button['height'] * 1.25), width=int(close_button['width'] * 1.25))

# create a rounded border for the button
close_button.config(background='white', foreground='black',
                    borderwidth=4, relief='raised', font=('Arial', 10, 'bold'))
    ...
    for count, row in enumerate(callLog, start=1):
        ...
        if count == 1:
            ...
            new_filename = (
                f"{dest_folder}"
                + "/"
                + f"{row['date']}".replace("/", "-").replace(":", "-").replace(" ", "_")
                + "_"
                + f"{row['external_number']}.mp3"
            )
